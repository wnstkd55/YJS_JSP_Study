...
public class UserController {
	...
	private UserService1 userService1;
				//userService1은 인터페이스 이므로, 하위의 클래스 객체가
	...
	//3. 회원등록(insert)
	// 회원 등록 링클르 클릭했을때 : GET, insertUse.jsp 뷰 페이지로 전송
	...
	// 회원등록품에서 값을 넣고 전송 : DB에 저장 <==post
	@RequestMapping(value = "/insertUser.do", method = RequestMethod.POST)
	public String insertUser(UserVO vo) {
		System.out.println("회원가입 성공 : - insertUser() Method호출 - Mybatis");
	...
	public String updateView(UserVO vo) {
		...
		//회원 수정 페이지에서 값을 넣고 전송버튼을 눌렀을때 :POST
	
	public String updateUser(UserVO vo) {
		userService1.updateUser(vo);
		return "getUserList.do)";		//회원 수정후 리스트  페이지로 이동
	}
